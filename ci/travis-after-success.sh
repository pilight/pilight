...
if [[ "$COVERAGE" == "1" && "$TRAVIS_EVENT_TYPE" != "pull_request" ]]; then
    ...
    lcov -r coverage.info '/usr/include/*' '*433nano.c' '*433gpio.c' '*IRgpio.c' '*none.c' '*zwave.cpp' '*adhoc.c' '*firmware.c' '*relay.c' '*pushover.c' '*pushbullet.c' '*dht11.c' '*dht22.c' -o coverage-filtered.info
    lcov -r lua.info '/usr/local/lib/pilight*' -o lua-filtered.info
    rm -rf lua.info
