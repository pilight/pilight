...
static void create_payload(struct mdns_packet_t **pkt, int type) {
	struct mdns_payload_t *payload = NULL;
	unsigned int nr = 0, x = 0, i = 0;
	unsigned char foo = 0xF9;

	switch(type) {
		/*case 0: {
			nr = (*pkt)->nrqueries;
			if(((*pkt)->queries = MALLOC(sizeof(struct mdns_payload_t *)*nr)) == NULL) {
				OUT_OF_MEMORY
			}
		} break;*/
		case 1: {
			nr = (*pkt)->nranswers;
			if(((*pkt)->answers = MALLOC(sizeof(struct mdns_payload_t *)*nr)) == NULL) {
				OUT_OF_MEMORY
			}
		} break;
		case 2: {
			nr = (*pkt)->rr_add;
			if(((*pkt)->records = MALLOC(sizeof(struct mdns_payload_t *)*nr)) == NULL) {
				OUT_OF_MEMORY
			}
		} break;
	}
	...
	for(x=0;x<nr;x++) {
		...
		switch(x) {
			case 0: {
				if((payload->name = STRDUP("testname.local.foo")) == NULL) {
					OUT_OF_MEMORY
				}
				payload->type = 12;
				if((payload->data.domain = STRDUP("testname.local.foo")) == NULL) {
					OUT_OF_MEMORY
				}
			} break;
			case 1: {
				if((payload->name = STRDUP("pilight.local")) == NULL) {
					OUT_OF_MEMORY
				}
				payload->type = 33;
				payload->priority = 1;
				payload->weight = 1;
				payload->port = 80;
				if((payload->data.domain = STRDUP("pilight.daemon.local")) == NULL) {
					OUT_OF_MEMORY
				}
			} break;
			case 2: {
				if((payload->name = STRDUP("pilight.local1")) == NULL) {
					OUT_OF_MEMORY
				}
				payload->type = 1;
				payload->data.ip4[0] = 192;
				payload->data.ip4[1] = 168;
				payload->data.ip4[2] = 1;
				payload->data.ip4[3] = 2;
			} break;
			case 3: {
				if((payload->name = STRDUP("pilight.local1")) == NULL) {
					OUT_OF_MEMORY
			...
			case 4: {
				...
				payload->data.text.nrvalues = 2;
				if((payload->data.text.values = MALLOC(sizeof(char *)*payload->data.text.nrvalues)) == NULL) {
					OUT_OF_MEMORY
				}
				if((payload->data.text.values[0] = STRDUP("foo")) == NULL) {
					OUT_OF_MEMORY
				}
				if((payload->data.text.values[1] = STRDUP("lorem")) == NULL) {
					OUT_OF_MEMORY
				}
			} break;
		}
...
static void create_packet(struct mdns_packet_t *pkt) {
	...
	if(pkt->qr == 0) {
		...
		if((pkt->queries[0]->name = STRDUP("testname.local.foo")) == NULL) {
			OUT_OF_MEMORY
		}
		if((pkt->queries[1]->name = STRDUP("testname1.local1.foo")) == NULL) {
			OUT_OF_MEMORY
		}
		if((pkt->queries[2]->name = STRDUP("testname.local1.foo")) == NULL) {
			OUT_OF_MEMORY
		}
		pkt->queries[0]->qu = 0;
		pkt->queries[0]->type = 33;
		pkt->queries[0]->class = 1;
		pkt->queries[1]->qu = 0;
		pkt->queries[1]->type = 33;
		pkt->queries[1]->class = 1;
		pkt->queries[2]->qu = 0;
		pkt->queries[2]->type = 33;
		pkt->queries[2]->class = 1;
	} else if(pkt->qr == 1) {
		pkt->nrqueries = 0;
	}
	create_payload(&pkt, 1);
	create_payload(&pkt, 2);
				...
				for(x=0;x<pkt.nranswers;x++) {
					switch(x) {
						...
						case 1: {
							...
							if(pkt.qr == 0) {
								CuAssertIntEquals(gtc, 23, pkt.answers[x]->length);
							} else {
								CuAssertIntEquals(gtc, 23, pkt.answers[x]->length);
							}
						} break;
