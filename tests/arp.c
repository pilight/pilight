...
static void *arp_found(int reason, void *param) {
...
static void test_arp(CuTest *tc) {
	...
	eventpool_callback(REASON_ARP_FOUND_DEVICE, arp_found);
	eventpool_callback(REASON_ARP_CHANGED_DEVICE, arp_update);
	eventpool_callback(REASON_ARP_LOST_DEVICE, arp_lost);
