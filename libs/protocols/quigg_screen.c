...
static void quiggScreenCreateMessage(int id, int state, int unit, int all) {
	...
	if (state==0) json_append_member(quigg_screen->message, "state", json_mkstring("up"));
	if (state==1) json_append_member(quigg_screen->message, "state", json_mkstring("down"));
}
...
// Byte 1,2 in raw buffer is the first logical byte, rawlen-3,-2 is the parity bit, rawlen-1 is the footer
	for(x=0; x<quigg_screen->rawlen-1; x+=2) {
		if(quigg_screen->raw[x+1] > PULSE_QUIGG_50) {
...
static void quiggScreenCreateZero(int s, int e) {
	int i;
	for(i=s;i<=e;i+=2) {
		quigg_screen->raw[i] = PULSE_QUIGG_SHORT;
		quigg_screen->raw[i+1] = PULSE_QUIGG_LONG;
	}
}
...
static void quiggScreenCreateOne(int s, int e) {
	int i;
	for(i=s;i<=e;i+=2) {
		quigg_screen->raw[i] = PULSE_QUIGG_LONG;
		quigg_screen->raw[i+1] = PULSE_QUIGG_SHORT;
	}
}

static void quiggScreenCreateHeader(void) {
	quigg_screen->raw[0] = PULSE_QUIGG_SHORT;
}

static void quiggScreenCreateFooter(void) {
	quigg_screen->raw[quigg_screen->rawlen-1] = PULSE_QUIGG_FOOTER;
}
...
static void quiggScreenCreateState(int state) {
	if(state==1) quiggScreenCreateOne(31, 32);	// dim down
}
...
static void quiggScreenCreateParity(void) {
	...
	for(i=25;i<=37;i+=2) {
		if(quigg_screen->raw[i] == PULSE_QUIGG_LONG) {
			p = -p;
		}
	}
	if(p==-1) {
...
static int quiggScreenCreateCode(JsonNode *code) {
	...
	if(json_find_number(code, "up", &itmp) == 0) state=0;
	if(json_find_number(code, "down", &itmp) == 0) state=1;
...
void quiggScreenInit(void) {
	...
	protocol_plslen_add(quigg_screen, (int)PULSE_QUIGG_FOOTER/PULSE_DIV);	// SHORT: GT-FSI-04a range: 620... 960
