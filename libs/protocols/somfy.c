...
#define MAXRAWLEN_SOMFY_PROT	241	// all Data bit equal new egenration: 14+1+2*56+2*56+2=241 241-4-4=233
// to be implemented
	...
	while (pRaw<=somfy->rawlen) {
		switch (protocol_sync) {
			...
			// No Data pulse, Sync only
				rDataTime=0;
				logprintf(LOG_DEBUG, "somfy: First pulse: Sync detected");
			...
			// if it is 1st pulse, we are now in the middile of the pulse windows
				logprintf(LOG_DEBUG, "somfy: First pulse: Data detected");
			...
			if( somfy->raw[pRaw] > PULSE_SOMFY_FOOTER_L && somfy->raw[pRaw] < PULSE_SOMFY_FOOTER_H) {
				if (pBin==56) {
					...
					logprintf(LOG_DEBUG, "somfy: prot 2a");
				} else {
					if (pBin==56) {
						protocol_sync=3;	// We received all 56 classic frame bits
						logprintf(LOG_DEBUG, "somfy: prot 2b");
					} else {
			...
			rDataTime = 0;
				...
				if ( pBin >= BINLEN_SOMFY_CLASSIC) {
					...
					logprintf(LOG_DEBUG, "somfy: prot 3");
				}
																				}
			break;
			case 3:
			...
			if( somfy->raw[pRaw] > PULSE_SOMFY_FOOTER_L && somfy->raw[pRaw] < PULSE_SOMFY_FOOTER_H) {
				if (pBin==112) {
					...
					logprintf(LOG_DEBUG, "somfy: prot 4");
				} else {
					protocol_sync=97;	// We have to check why we end up here
					logprintf(LOG_ERR, "somfy: Err 31. Payload analysis required.");
				}
				break;
			}
			rDataTime= rDataTime+somfy->raw[pRaw];
			...
			logprintf(LOG_DEBUG, "somfy: End of new generation data. pulse: %d - pRaw: %d - bin: %d", somfy->raw[pRaw], pRaw, pBin);
			if ( (somfy->raw[pRaw] > PULSE_SOMFY_FOOTER_L) && (somfy->raw[pRaw] < PULSE_SOMFY_FOOTER_H) ) {
				protocol_sync = 98;
			} else {
				protocol_sync = 5;
				logprintf(LOG_DEBUG, "somfy: prot 5");
			}
			break;
			case 5:
			...
			break;
			case 96:
			logprintf(LOG_ERR, "somfy: Err 96 unexpected EOF.");
			case 97:
...
void somfyScreenInit(void) {
	...
	options_add(&somfy->options, 'a', "address", OPTION_HAS_VALUE, DEVICES_ID, JSON_NUMBER, NULL, "^([0-9]{1,6}|[1-5][0-9]{6}|16[0-6][0-9]{5}|167[0-6][0-9]{4}|1677[0-6][0-9]{3}| 16777[0-1][0-9]{2}|1677720[0-9]|1677721[0-6])$");
	options_add(&somfy->options, 'c', "rollingcode", OPTION_HAS_VALUE, DEVICES_VALUE, JSON_NUMBER, NULL, "^([0-9]{1,4}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])$");
	options_add(&somfy->options, 'k', "rollingkey", OPTION_HAS_VALUE, DEVICES_VALUE, JSON_NUMBER, NULL, "^([0-9]|1[0-5])$");
	options_add(&somfy->options, 'n', "command_code", OPTION_HAS_VALUE, DEVICES_VALUE, JSON_NUMBER, NULL, "^([0-9]|1[0-5])$");
	options_add(&somfy->options, 't', "up", OPTION_NO_VALUE, DEVICES_STATE, JSON_STRING, NULL, NULL);
	options_add(&somfy->options, 'f', "down", OPTION_NO_VALUE, DEVICES_STATE, JSON_STRING, NULL, NULL);
	options_add(&somfy->options, 'm', "my", OPTION_NO_VALUE, DEVICES_STATE, JSON_STRING, NULL, NULL);
	options_add(&somfy->options, 'g', "prog", OPTION_NO_VALUE,DEVICES_STATE, JSON_STRING, NULL, NULL);

	options_add(&somfy->options, 0, "readonly", OPTION_HAS_VALUE, GUI_SETTING, JSON_NUMBER, (void *)0, "^[10]{1}$");
