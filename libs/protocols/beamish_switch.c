...
static void beamish_SwCreateMessage(int id, int unit, int state, int all) {
...
static void beamish_SwParseCode(void) {
	int i = 0, x = 0;
	...
	if (code == 192){ unit = 1; }
	else if (code == 48){ unit = 2; }
	else if (code == 12){ unit = 3; }
	else if (code == 3){ unit = 4; }
	else if (code == 15){ unit = 5; }
	else if (code == 195){ unit = 6; }
	...
	beamish_SwCreateMessage(id, unit, state, all);
}

static void beamish_SwCreateHigh(int s, int e) {
	int i;
...
static void beamish_SwCreateLow(int s, int e) {
	int i;
...
static void beamish_SwClearCode(void) {
	beamish_SwCreateHigh(0,47);
}

static void beamish_SwCreateId(int id) {
	int binary[255];
	int length = 0;
	int i=0, x=0;
	...
	for(i=0;i<=length;i++) {
		if(binary[i]==1) {
			x=i*2;
			beamish_SwCreateLow(31-(x+1), 31-x);
		}
	}
}

static void beamish_SwCreateUnit(int unit) {
	int binary[255];
	int length = 0;
	int i=0, x=0;
	...
	for(i=0;i<=length;i++) {
		if(binary[i]==1) {
			x=i*2;
			beamish_SwCreateLow(47-(x+1), 47-x);
		}
	}
}

static void beamish_SwCreateFooter(void) {
	beamish_switch->raw[48]=(beamish_switch->plslen->length);
	beamish_switch->raw[49]=(PULSE_DIV*beamish_switch->plslen->length);
}

static int beamish_SwCreateCode(JsonNode *code) {
	int id = -1;
	int unit = -1;
	int state = -1;
	int all = 0;
	double itmp = -1;
	int lookup_button_code [7]={0,192,48,12,3,15,195};
	...
	if(id == -1 || (unit == -1 && all == 0) || (state == -1 && all == 1)) {
		...
		beamish_SwCreateMessage(id, unit, state, all);
		beamish_SwClearCode();
		beamish_SwCreateId(id);
		unit = lookup_button_code[unit];
		beamish_SwCreateUnit(unit);
		beamish_SwCreateFooter();
	}
	return EXIT_SUCCESS;
}

static void beamish_SwPrintHelp(void) {
...
void beamish_SwInit(void) {
	...
	beamish_switch->parseCode=&beamish_SwParseCode;
	beamish_switch->createCode=&beamish_SwCreateCode;
	beamish_switch->printHelp=&beamish_SwPrintHelp;
}
