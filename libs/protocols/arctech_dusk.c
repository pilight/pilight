...
static void arctechDuskCreateMessage(int id, int unit, int state, int all) {
	...
	json_append_member(arctech_dusk->message, "id", json_mknumber(id));
	if(all == 1) {
		json_append_member(arctech_dusk->message, "all", json_mknumber(all));
	} else {
		json_append_member(arctech_dusk->message, "unit", json_mknumber(unit));
	}
...
void arctechDuskInit(void) {
	...
	options_add(&arctech_dusk->options, 'u', "unit", OPTION_HAS_VALUE, CONFIG_ID, JSON_NUMBER, NULL, "^([0-9]{1}|[1][0-5])$");
	options_add(&arctech_dusk->options, 'i', "id", OPTION_HAS_VALUE, CONFIG_ID, JSON_NUMBER, NULL, "^([0-9]{1,7}|[1-5][0-9]{7}|6([0-6][0-9]{6}|7(0[0-9]{5}|10([0-7][0-9]{3}|8([0-7][0-9]{2}|8([0-5][0-9]|6[0-3]))))))$");
	options_add(&arctech_dusk->options, 't', "dusk", OPTION_NO_VALUE, CONFIG_STATE, JSON_STRING, NULL, NULL);
	options_add(&arctech_dusk->options, 'f', "dawn", OPTION_NO_VALUE, CONFIG_STATE, JSON_STRING, NULL, NULL);

	options_add(&arctech_dusk->options, 'a', "all", OPTION_HAS_VALUE, CONFIG_SETTING, JSON_NUMBER, (void *)0, "^[10]{1}$");
	options_add(&arctech_dusk->options, 0, "gui-readonly", OPTION_HAS_VALUE, CONFIG_SETTING, JSON_NUMBER, (void *)0, "^[10]{1}$");
