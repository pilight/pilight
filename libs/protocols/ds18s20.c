...
void *ds18s20Parse(void *param) {
				...
				while((file = readdir(d)) != NULL) {
					if(file->d_type == DT_REG) {
						if(strcmp(file->d_name, "w1_slave") == 0) {
							...
							ds18s20_w1slave = realloc(ds18s20_w1slave, w1slavelen);
	...
	if(id) sfree((void *)&id);
	if(ds18s20_w1slave) sfree((void *)&ds18s20_w1slave);
	ds18s20_nrfree--;
...
void ds18s20Init(void) {
	...
	options_add(&ds18s20->options, 't', "temperature", has_value, config_value, "^[0-9]{1,5}$");
	options_add(&ds18s20->options, 'i', "id", has_value, config_id, "^[a-z0-9]{12}$");
	...
	protocol_setting_add_number(ds18s20, "interval", 5);
