...
function M.check(parameters)
	if parameters['SUBJECT'] == nil then
		pilight.log(LOG_ERR, "sendmail action is missing a \"SUBJECT\" statement");
	end

	if parameters['MESSAGE'] == nil then
		pilight.log(LOG_ERR, "sendmail action is missing a \"MESSAGE\" statement");
	end

	if parameters['TO'] == nil then
		pilight.log(LOG_ERR, "sendmail action is missing a \"TO\" statement");
	end

	if #parameters['SUBJECT']['value'] ~= 1 or parameters['SUBJECT']['value'][2] ~= nil then
		pilight.log(LOG_ERR, "sendmail action \"SUBJECT\" only takes one argument");
	end

	if #parameters['MESSAGE']['value'] ~= 1 or parameters['MESSAGE']['value'][2] ~= nil then
		pilight.log(LOG_ERR, "sendmail action \"MESSAGE\" only takes one argument");
	end

	if #parameters['TO']['value'] ~= 1 or parameters['TO']['value'][2] ~= nil then
		pilight.log(LOG_ERR, "sendmail action \"TO\" only takes one argument");
	end

	if parameters['MESSAGE']['value'][1] == "." then
		pilight.log(LOG_ERR, "sendmail action \"MESSAGE\" cannot be a single dot");
	end

	local address = parameters['TO']['value'][1];
	if string.match(address, "^[%w.]+@[%w]+[.][%w]?[%w]?[%w]?$") ~= address then
		pilight.log(LOG_ERR, "sendmail action \"TO\" must contain an e-mail address");
	end

	local config = pilight.config();

	if config.getSetting("smtp-host") == nil then
		pilight.log(LOG_ERR, "sendmail: setting \"smtp-host\" is missing in config");
	end

	if config.getSetting("smtp-port") == nil then
		pilight.log(LOG_ERR, "sendmail: setting \"smtp-port\" is missing in config");
	end

	if config.getSetting("smtp-user") == nil then
		pilight.log(LOG_ERR, "sendmail: setting \"smtp-user\" is missing in config");
	end

	if config.getSetting("smtp-password") == nil then
		pilight.log(LOG_ERR, "sendmail: setting \"smtp-password\" is missing in config");
	end

	local address = config.getSetting("smtp-sender");
	if address == nil then
		pilight.log(LOG_ERR, "sendmail: setting \"smtp-sender\" is missing in config");
	end

	if string.match(address, "^[%w.]+@[%w]+[.][%w]?[%w]?[%w]?$") ~= address then
		pilight.log(LOG_ERR, "sendmail action \"FROM\" must contain an e-mail address");
	end
...
function M.callback(mail)
	if mail.getStatus() == true then
		pilight.log(LOG_NOTICE, "successfully sent mail with subject \"" .. mail.getSubject() .. "\" to " .. mail.getTo());
	else
		pilight.log(LOG_NOTICE, "failed sent mail with subject \"" .. mail.getSubject() .. "\" to " .. mail.getTo());
	end
end
