...
static void createMessage(unsigned int homeId, int nodeId, int state) {
...
static void parseCommand(struct JsonNode *code) {
	...
	if(cmdId == COMMAND_CLASS_SWITCH_BINARY) {
		createMessage(homeId, nodeId, value);
	}
}
...
static int createCode(struct JsonNode *code) {
	...
	if(state == 0) {
		zwaveSetValue(nodeId, COMMAND_CLASS_SWITCH_BINARY, "false");
	} else {
		zwaveSetValue(nodeId, COMMAND_CLASS_SWITCH_BINARY, "true");
	}
	
	createMessage(homeId, nodeId, state);		
